...
Pre-1.0. Requests that omit `X-Islo-Api-Version` still get server behavior `2026-02-23`. `NewIslo` and `NewClient` send `2026-09-15` by default. Pass `option.WithAPIVersion("2026-02-23")` to keep the previous server behavior.

Client breaks in this release:

- `ListAllJobRuns` and `ListFactoryLineRuns` return cursor pages instead of slices.
- Custom disconnect takes the stable provider slug instead of a Descope app id.
- `JobRunStatus` is removed. Status filters are plain strings.
- Path fields and bodies are separate on request structs. `TriggerJobRun` takes `*TriggerJobRunRequest` (`Name` plus `Body *JobRunCreate`), not `*JobRunCreate`.
- Module requires Go 1.21. Resource clients are constructed with `NewClient(options *core.RequestOptions)`, not variadic options. Keep using `client.NewIslo`.
